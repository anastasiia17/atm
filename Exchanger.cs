...
namespace bankomat
{
    public class ExchangeCoins
    {
        ...
        public ExchangeCoins(int[] banknotes)
        {
        ...
        public List<string> Exchange(int sum)
        {
            ...
            Combinations(sum, banknotes.Length - 1, "");
            return listOfCombinations;
        }

        private void Combinations(int sum, int coinIndex, string combination)
        {
            if (sum < 0)
            {
                return;
            ...
            {
                Combinations(sum - i * banknotes[coinIndex], coinIndex - 1, combination + LastCombination(i, coinIndex));
            }
